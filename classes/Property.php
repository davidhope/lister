	...
	public function get($propertyId){
      ...
      from property where propertyId = :propertyId;");
			...
			if($propertyId > 0){
				...
				$parcel = new Parcel();
				$Property->parcel =  $parcel->getByProperty($Property->$propertyId);
			}
			return $Property;
		}catch(PDOException $pdoe){
			throw new Exception($pdoe->getMessage());
	...
	public function save(){
		...
		try {
			if($this->propertyId > 0){
					...
					$pdo = getPDO();
				$stmt =  $pdo->prepare("insert into property(propertyTypeId,neighborhoodId,zoningTypeId,areaCode,subdivision,location,latitude,longitude,elevation,unit,address,city,stateId,zip,county,gated,floor,bed,bath,stories,garage,pool,spa,yearBuilt,schoolElementary,schoolMiddle,schoolHigh,sqFtLiving,sqFtLot,acres,lastUpdateDate,lastUpdateId)values(:propertyTypeId,:neighborhoodId,:zoningTypeId,:areaCode,:subdivision,:location,:latitude,:longitude,:elevation,:unit,:address,:city,:stateId,:zip,:county,:gated,:floor,:bed,:bath,:stories,:garage,:pool,:spa,:yearBuilt,:schoolElementary,:schoolMiddle,:schoolHigh,:sqFtLiving,:sqFtLot,:acres,:lastUpdateDate,:lastUpdateId);");
					if(is_null($this->propertyTypeId)){
						$stmt->bindValue(':propertyTypeId',NULL, PDO::PARAM_INT);
					} else {
						$stmt->bindParam(':propertyTypeId',$this->propertyTypeId, PDO::PARAM_INT);
